...
import { Icon, fmt, riskCls, trendArrow } from './core.jsx';
...
// Filter bar — onExportPdf and onExportCsv are optional callbacks.
// If not provided, the PDF/Export buttons call onStub (which shows "in dev" toast).
export function FilterBar({ date, setDate, sectionsFilter, setSectionsFilter, workFilter, setWorkFilter, onReset, onExportPdf, onExportCsv, onStub }){
  ...
  return (
    <div className="filters">
      ...
      {MOCK.sections.map(s => (
        <span key={s.id}
              className={"chip " + (sectionsFilter.includes(s.id)?"on":"")}
              onClick={()=>toggle(setSectionsFilter, sectionsFilter, s.id)}>
          {"\u2116"}{s.id}
        </span>
      ))}
