/* ==================================================================
   ...
   Условные значения; структура соответствует реальной предметной области.
   ================================================================== */
(function(){
  const sections = [
  ...
  // Прогресс по типам работ, факт за сутки
  const metrics = [
    /* уч  пес  щпгс  св.осн  св.пр   пес24  щпгс24  свОсн24 свПр24  проблемы  риск */
  ...
  // Суточный отчёт по карьерам (rides, trucks, output by shift)
  function rowFor(sec){
  ...
  // Объекты (мосты/путепроводы/трубы/свайные поля/пересечения) для карты
  const objects = [
  ...
  // Производительность техники — матрица 3 × 8 (% от нормы)
  ...
  // Гантт — по участку: типы работ с датами
  function gantt(sec){
    // start/end как % по временной оси (01 мар — 31 окт 2026)
    const base = sec.id; // для вариации
    return [
  ...
  // Тренд: объёмы перевозок за 30 дней с разбивкой по контрагентам
  const trendDays = [];
  for (let i=0; i<30; i++){
  ...
  // Разбивка суточной возки песка по участкам × контрагентам
  ...
  // ─────── Возка: матрица карьер × участок × подрядчик × материал ───────
  // Имитация material_movements с делением на три силы (ЖДС/АЛМАЗ/наёмники)
  const contractors = [
    { id:"zhds",  name:"ЖДС (собственные)",        short:"ЖДС",    kind:"own",  color:"#1a1a1a" },
    { id:"almaz", name:"ООО «АЛМАЗ»",              short:"АЛМАЗ",  kind:"own",  color:"#dc2626" },
    { id:"hire",  name:"Прочие наёмники",          short:"Наёмн.", kind:"hire", color:"#7f1d1d" },
  ];
  const materialFlow = [];
  quarries.forEach((q, qi) => {
    [1,2,3,4,5,6,7,8].forEach(sec => {
      // карьер возит только на свой основной участок + возможно соседние
      const weight = sec === q.sec ? 1.0 : (Math.abs(sec - q.sec) === 1 ? 0.25 : 0);
      if (!weight) return;
      contractors.forEach((c, ci) => {
        const share = c.kind === "own"
          ? (ci === 0 ? 0.45 : 0.32)   // ЖДС + АЛМАЗ
          : 0.23;                       // наёмники
        const base = q.arm * 9 + qi * 180 + sec * 140;
        const volume = Math.round(base * weight * share);
        const trips = Math.max(1, Math.round(volume / 16));
        if (volume > 0) {
          materialFlow.push({
            quarry: q.name, material: q.mat, arm: q.arm,
            section: sec, contractor: c.id, contractorName: c.name, contractorKind: c.kind,
            volume, trips
          });
        }
      });
  ...
  // ─────── Производительность техники: порядок самосвал/экск/бульдозер/автогрейдер/каток ───────
  const equipmentTypes = [
    { code:"dump",    label:"Самосвал",     norm: 128,  unit:"м³/сут",   metric:"volume",  note:"16 м³/рейс × 2 см × 4 рейса" },
    { code:"excav",   label:"Экскаватор",   norm: 1038, unit:"м³/сут",   metric:"volume",  note:"ковш 2 м³ × 64 цикла/ч × 8.1 ч" },
    { code:"dozer",   label:"Бульдозер",    norm: 720,  unit:"м³/сут",   metric:"volume",  note:"отвал 3.4 м³ × плечо 30 м" },
    { code:"grader",  label:"Автогрейдер",  norm: 4200, unit:"м²/сут",   metric:"area",    note:"планировка ширина 3.6 м × 1.2 км/ч" },
    { code:"roller",  label:"Каток",        norm: 3800, unit:"м²/сут",   metric:"area",    note:"ширина 2.1 м × 4 прохода × 6 км/ч" },
  ];
  const equipmentUnitsBySec = {
    dump:   [0, 18, 14, 22, 16, 12, 20, 15, 18],
    excav:  [0, 4,  3,  5,  4,  3,  4,  3,  4],
    dozer:  [0, 2,  2,  3,  2,  2,  3,  2,  2],
    grader: [0, 1,  1,  2,  1,  1,  1,  1,  1],
    roller: [0, 2,  2,  3,  2,  1,  2,  2,  2],
  };
  // Пять строк по типам, 8 колонок по участкам (% от нормы)
  const equipmentMatrixNew = {
    rows: equipmentTypes,
    cols: sections.map(s => "№"+s.id),
    values: [
      [ 88, 104, 72, 112, 96, 82, 101, 58],  // Самосвал
      [ 92, 108, 68, 116, 101, 90, 105, 64], // Экскаватор
      [ 75,  98, 64, 108, 92, 78, 96,  60],  // Бульдозер
      [ 82,  95, 70, 110, 88, 74, 99,  62],  // Автогрейдер
      [ 70,  92, 62, 104, 85, 72, 94,  55],  // Каток
    ],
    unitsBySec: equipmentUnitsBySec,
  };

  // ─────── Свайные работы: план (из project_work_items) и факт на дату/период ───────
  const pilesPlanFact = sections.map(m => {
    const planMain  = 420 + m.id * 30;
    const planTest  = 36  + m.id * 3;
    const planTests = 18  + m.id * 2;
    const factMain  = Math.round(planMain  * (metrics.find(x=>x.id===m.id).pileMain / 100));
    const factTest  = Math.round(planTest  * (metrics.find(x=>x.id===m.id).pileTrial/ 100));
    const factTests = Math.round(planTests * (metrics.find(x=>x.id===m.id).pileMain / 100));
    // суточные дельты (текущая дата)
    const todayMain  = metrics.find(x=>x.id===m.id).pMain24;
    const todayTest  = metrics.find(x=>x.id===m.id).pTr24;
    const todayTests = Math.round(todayMain * 0.18);
    return {
      sec: m.id,
      planMain, factMain, todayMain,
      planTest, factTest, todayTest,
      planTests, factTests, todayTests,
    };
  });

  // ─────── Техника на пикетах (для карты) ───────
  // имитация report_equipment_units — по каждому отчёту агрегат по pk × типу
  const equipmentOnMap = [];
  objects.forEach(o => {
    const m = String(o.pk).match(/ПК(\d+)/);
    if (!m) return;
    const pk = Number(m[1]);
    // на объекте работает 1-2 типа техники
    const rand = (pk * 17) % 10;
    if (o.kind === "pile") {
      equipmentOnMap.push({ pk, sec: o.sec, type:"excav", count: 2 + (rand%2) });
      equipmentOnMap.push({ pk, sec: o.sec, type:"dump",  count: 4 + (rand%3) });
    } else if (o.kind === "bridge" || o.kind === "putoprovod") {
      equipmentOnMap.push({ pk, sec: o.sec, type:"excav", count: 1 + (rand%2) });
      equipmentOnMap.push({ pk, sec: o.sec, type:"dump",  count: 3 + (rand%4) });
    }
  });
  // добавим работы по отсыпке на случайных ПК в каждом участке
  tads.forEach((t, i) => {
    const m = String(t.pk).match(/ПК(\d+)/);
    if (!m) return;
    const pk = Number(m[1]);
    if (t.pi > 0 || t.wr > 0) {
      equipmentOnMap.push({ pk, sec: t.sec, type:"dump",   count: 5 + (i%4) });
      equipmentOnMap.push({ pk, sec: t.sec, type:"excav",  count: 2 + (i%3) });
      equipmentOnMap.push({ pk, sec: t.sec, type:"dozer",  count: 1 + (i%2) });
      if (i%3===0) equipmentOnMap.push({ pk, sec: t.sec, type:"grader", count: 1 });
      if (i%2===0) equipmentOnMap.push({ pk, sec: t.sec, type:"roller", count: 1 + (i%2) });
    }
  });
  ...
  window.MOCK = {
    ...
    contractors, materialFlow,
    equipmentTypes, equipmentMatrixNew,
    pilesPlanFact, equipmentOnMap,
    overall: overall()
  };
})();
